...
def list_images(directory):
    '''
    :param directory: 路径
    :return images: 参数路径下的图像绝对地址
    :return names: 参数路径下图像的名称
    '''
    images = []
    names = []
...
def load_dataset(image_path, BATCH_SIZE, num_imgs=None):
    '''
    将图像数据路径去掉多于batch整除的数据，并打乱
    :param image_path: (list)图像数据绝对路径
    :param BATCH_SIZE: (int)batch大小
    :param num_imgs: 作为训练数据的大小，默认为传入全部数据
    :return original_imgs_path:  (list)图像训练数据绝对路径
    :return batches: (int)训练数据共多少个batch
    '''
    if num_imgs is None:
...
def get_image(path, height=256, width=256, flag=False):
    '''
    根据路径读取数据，并对图像进行resize
    :param path: (list)读取路径
    :param height: 读取图像后resize的高度
    :param width: 读取图像后resize的宽度
    :param flag: 读取方式，默认为灰度图
    :return:
    '''
    if flag is True:
